...
public class BlockService {
    ...
    public String createGenesisBlock() {
        ...
        while (true) {
            // 计算新区块hash值
            newBlockHash = blockService.calculateHash("createBlock", tsaList, nonce);
            // 校验hash值
     ...
     *测试无效区块时使用
     */
    public Block createNewBlockTest(int nonce, String previousHash, String hash, List<Transaction> blockTxs) {
        Block block = new Block();
        block.setIndex(blockCache.getBlockChain().size() + 1);
        //时间戳
        block.setTimestamp(System.currentTimeMillis());
    ...
    public String calculateHash2(String previousHash, List<BlockData> blockDataList, int nonce) {
        return CryptoUtil.SHA256(previousHash + JSON.toJSONString(blockDataList) + nonce);
    }
