package blockchain.web.service;

import java.util.ArrayList;
import java.util.List;

import blockchain.web.model.Block;
import blockchain.web.model.Message;
import blockchain.web.model.Transaction;
import blockchain.web.util.BlockCache;
import blockchain.web.util.BlockConstant;
import blockchain.web.util.CommonUtil;
...
public class PowService {
    ...
    public Block mine(){
        ...
        // 创建新的区块
        Block block = blockService.createNewBlockTest(nonce, blockCache.getLatestBlock().getHash(), newBlockHash, tsaList);
    ...
    public Block invokeBlockService(Transaction transaction){
        ...
        while (true) {
            // 计算新区块hash值
            newBlockHash = blockService.calculateHash(blockCache.getLatestBlock().getHash(), transactionList, nonce);
            // 校验hash值
            if (blockService.isValidHash(newBlockHash)) {
                System.out.println("挖矿完成，正确的hash值：" + newBlockHash);
                System.out.println("挖矿耗费时间：" + (System.currentTimeMillis() - start) + "ms");
        ...
        // 创建新的区块
        Block block = blockService.createNewBlock(nonce, blockCache.getLatestBlock().getHash(), newBlockHash, transactionList);
